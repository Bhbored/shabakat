...
class PricePerAmpPreferenceScreen extends ConsumerStatefulWidget {
  const PricePerAmpPreferenceScreen({super.key});

  @override
  ConsumerState<PricePerAmpPreferenceScreen> createState() =>
      _PricePerAmpPreferenceScreenState();
}

class _PricePerAmpPreferenceScreenState
    extends ConsumerState<PricePerAmpPreferenceScreen> {
  ...
  double _valueForTier(CompanyPreferences preferences, _CustomerTier tier) {
    return switch (tier) {
      _CustomerTier.base => preferences.pricePerAmp,
      _CustomerTier.residential => preferences.residentialPricePerAmp,
      _CustomerTier.commercial => preferences.commercialPricePerAmp,
      _CustomerTier.industrial => preferences.industrialPricePerAmp,
    };
  }
  ...
  CompanyPreferences _updateTierValue(
    ...
    return switch (tier) {
      _CustomerTier.base => preferences.copyWith(pricePerAmp: value),
      _CustomerTier.residential => preferences.copyWith(
        residentialPricePerAmp: value,
      ),
      _CustomerTier.commercial => preferences.copyWith(
        commercialPricePerAmp: value,
      ),
      _CustomerTier.industrial => preferences.copyWith(
        industrialPricePerAmp: value,
      ),
    };
  }
            ...
            child: SizedBox(
              ...
              child: ElevatedButton(
                onPressed: isSaving
                    ...
                    ? () async {
                        _commitCurrentValue();
                        final ampDraft = _draft!;
                        if (ampDraft == _original) return;

                        final updated = current.copyWith(
                          pricePerAmp: ampDraft.pricePerAmp,
                          residentialPricePerAmp:
                              ampDraft.residentialPricePerAmp,
                          commercialPricePerAmp: ampDraft.commercialPricePerAmp,
                          industrialPricePerAmp: ampDraft.industrialPricePerAmp,
                        );

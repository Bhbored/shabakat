...
class FixedChargePreferenceScreen extends ConsumerStatefulWidget {
  const FixedChargePreferenceScreen({super.key});

  @override
  ConsumerState<FixedChargePreferenceScreen> createState() =>
      _FixedChargePreferenceScreenState();
}

class _FixedChargePreferenceScreenState
    extends ConsumerState<FixedChargePreferenceScreen> {
  ...
  double _valueForTier(CompanyPreferences preferences, _CustomerTier tier) {
    return switch (tier) {
      _CustomerTier.base => preferences.fixedCharge,
      _CustomerTier.residential => preferences.residentialFixedCharge,
      _CustomerTier.commercial => preferences.commercialFixedCharge,
      _CustomerTier.industrial => preferences.industrialFixedCharge,
    };
  }
  ...
  CompanyPreferences _updateTierValue(
    ...
    return switch (tier) {
      _CustomerTier.base => preferences.copyWith(fixedCharge: value),
      _CustomerTier.residential => preferences.copyWith(
        residentialFixedCharge: value,
      ),
      _CustomerTier.commercial => preferences.copyWith(
        commercialFixedCharge: value,
      ),
      _CustomerTier.industrial => preferences.copyWith(
        industrialFixedCharge: value,
      ),
    };
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text('settings.preferences.fixed_charge'.tr()),
      ),
      body: Column(
        children: [
          Expanded(
            ...
            child: SizedBox(
              ...
              child: ElevatedButton(
                onPressed: isSaving
                    ...
                    ? () async {
                        _commitCurrentValue();
                        final chargeDraft = _draft!;
                        if (chargeDraft == _original) return;

                        final updated = current.copyWith(
                          fixedCharge: chargeDraft.fixedCharge,
                          residentialFixedCharge:
                              chargeDraft.residentialFixedCharge,
                          commercialFixedCharge:
                              chargeDraft.commercialFixedCharge,
                          industrialFixedCharge:
                              chargeDraft.industrialFixedCharge,
                        );
